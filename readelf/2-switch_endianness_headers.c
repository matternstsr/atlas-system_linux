...
void switch_endianness_program_headers_64(elf_t *elf_header, size_t index)
{
	...
	elf_header->p64[index].p_offset  = seu64(elf_header->p64[index].p_offset);
	elf_header->p64[index].p_vaddr   = seu64(elf_header->p64[index].p_vaddr);
	elf_header->p64[index].p_paddr   = seu64(elf_header->p64[index].p_paddr);
	elf_header->p64[index].p_filesz  = seu64(elf_header->p64[index].p_filesz);
	elf_header->p64[index].p_memsz   = seu64(elf_header->p64[index].p_memsz);
	elf_header->p64[index].p_align   = seu64(elf_header->p64[index].p_align);
