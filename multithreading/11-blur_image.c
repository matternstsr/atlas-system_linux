...
static void* thread_blur(void* arg)
{
    ...
    apply_gaussian_blur(data->img, data->img_blur, data->kernel, data->x_start,
						data->y_start, data->width, data->height);
    return NULL;
}
...
void blur_image(img_t *img_blur, img_t const *img, kernel_t const *kernel)
{
    ...
    for (size_t i = 0; i < num_threads; i++)
	{
        ...
        thread_data[i].width = (i == num_threads - 1) ?
								(img->w - i * portion_width) : portion_width;
