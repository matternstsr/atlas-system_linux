...
pixel_t apply_kernel(const img_t *img, const kernel_t *kernel, size_t x, size_t y)
{
	...
	blurred_pixel.r = (uint8_t)fminf(fmaxf(sum_r, 0), 255);
	blurred_pixel.g = (uint8_t)fminf(fmaxf(sum_g, 0), 255);
	blurred_pixel.b = (uint8_t)fminf(fmaxf(sum_b, 0), 255);
