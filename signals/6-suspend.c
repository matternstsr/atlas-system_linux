 ...
 * by printing "Caught %d" where %d is the signal number received,
 * followed by "Signal received". It then restores
 ...
 * and handles the SIGINT signal once by calling sigint_handler.
 * After handling SIGINT, it suspends the program until another signal
 * is received.
 */
 ...
 *
 * This function is the entry point of the program. It calls
 * handle_sigint_and_exit to set up the SIGINT signal handler and handle it
 * once. After handling SIGINT, the function returns EXIT_SUCCESS.
 *
