...
void print_long_list(file_node_t *file_list, ls_flag_t *flags)
{
	...
	if (flags->reversed) /* Check if printing should be reversed */
		while (file_list->next) /* Iterate to the last node if printing is reversed */
			file_list = file_list->next;
	while (file_list != NULL) /* Iterate through the file list */
	{
		if (should_print(file_list->name) == true) /* Check if the file should be printed */
		{
			perm_log(perms, file_list->info->st_mode); /* Get and log file permissions */
			last_modified(time, file_list->info->st_mtime); /* Get and log last modified time */
			...
			printf(str, perms, num_links, user, group, size, time, name); /* Print file info */
			if (S_ISLNK(file_list->info->st_mode) == true) /* Check if it's a symbolic link */
			{
				for (i = 0; i < 256; i++) /* Initialize symbolic link path buffer */
					sym_link_path[i] = '\0';
				sprintf(buf, "%s/%s", file_list->dir_name, file_list->name); /* Construct path to the symbolic link */
