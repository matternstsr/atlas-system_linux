...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {
        ...
        return EXIT_FAILURE; /* Return failure if less than 2 arguments provided */
    }
    /* Process each file provided as argument */
    for (current_file = file_names; *current_file != NULL; ++current_file) {
        exit_status += process_file(*current_file, (current_file != file_names), argv);
...
int process_file(char *file_name, int multiple, char **argv)
{
    ...
    if (exit_status != EXIT_SUCCESS) {
        ...
        fprintf(stderr, "%s: %s: no symbols found\n", argv[0], file_name);
    }
