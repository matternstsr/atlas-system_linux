...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        ...
        return EXIT_FAILURE; /* Return fail if less than 2 arguments provided */
    }
...
int process_file(char *file_name, int multiple, char **argv)
{
    ...
    if (IS_32(elf_header.e64)) /* Check if ELF file is 32-bit */
    {
        ...
        r = read(fd, &elf_header.e32, sizeof(elf_header.e32)); /*Read32ELFhdr*/
						...
						!is_elf_file((char *)&elf_header.e32))
        {
            fprintf(stderr,
                    ...
                    argv[0], file_name);
            close(fd);
            return EXIT_FAILURE; /* Return failure if 32 ELF hdr is not valid */
        }
    }
    ...
    exit_status = process_file_helper(&elf_header, fd, &num_printed, argv);
...
/**
 * process_file_helper - Helper to process ELF file after header is read.
 * @elf_header: Pointer to elf_t structure containing ELF header information
 * @fd: File descriptor of the ELF file
 * @num_printed: Pointer to size_t variable to count printed symbols
 * @argv: Argument vector passed to main
 *
 * Return: EXIT_SUCCESS on success, EXIT_FAILURE on failure
 */
