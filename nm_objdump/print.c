...
void print_section_headers32(elf_t *elf_header, char *string_table)
{
    ...
    for (i = 0; i < EGET(e_shnum); i++)
        printf(FORMAT_SECTION_32,
               ...
               string_table + SGET(i, sh_name),           /* Section name from string table */
...
int print_all_symbol_tables(elf_t *elf_header, int fd, size_t *num_printed)
{
    ...
    for (i = 0; i < EGET(e_shnum); i++)
    {
        ...
        {
            *num_printed += print_symbol_table(elf_header, fd, i, string_table);   /* Update printed symbol count */
        }
    }
                          ...
                          char *string_table)
{
    ...
    /* Determine ELF class (32-bit or 64-bit) and print symbol table accordingly */
    if (IS_64)
    {
        num_printed = print_symbol_table64(elf_header, string_table,
                                           sym_string_table, versym, verneed, verneed_size, i);
        elf_header->y64 = (free(elf_header->y64), NULL);   /* Free 64-bit symbol table */
    }
    else
    {
        num_printed = print_symbol_table32(elf_header, string_table,
                                           sym_string_table, versym, verneed, verneed_size, i);
        elf_header->y32 = (free(elf_header->y32), NULL);   /* Free 32-bit symbol table */
    }

    /* Free allocated memory for symbol string table, version needed table, and version symbol table */
                            ...
                            size_t verneed_size, int section)
{
    ...
    for (i = 0; i < size; i++)
    {
        /* Skip symbols of type STT_SECTION, STT_FILE, or the first symbol (index 0) */
        ...
        printf("%c %s\n",
               get_nm_type32(elf_header->y32[i], elf_header->s32),   /* Symbol type */
    ...
    /* The following lines are unreachable code; these parameters are not used in this function */
    (void)string_table;
    (void)versym;
    (void)verneed;
    (void)verneed_size;
                            ...
                            size_t verneed_size, int section)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        /* Print symbol type and name */
        printf("%c %s\n",
               get_nm_type64(elf_header->y64[i], elf_header->s64),   /* Symbol type */
               sym_string_table + YGET(i, st_name));                 /* Symbol name */
