...
/**
 * crack_open_file - attempts to open a file and returns the file descriptor; exits on failure
...
int crack_open_file(char *name, int silent, char **argv)
{
    ...
    if (fd == -1)
    {
        ...
        if (!silent)
        {
            ...
            {
                /* Print error message to stderr for no such file or directory error */
